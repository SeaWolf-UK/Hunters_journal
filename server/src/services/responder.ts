...
const DESCRIBE_TEMPLATES = [
  (c: Creature) => `Ah, the ${c.name}. ${c.lore_summary}`,
  (c: Creature) => `${c.name}. ${c.lore_summary}`,
  (c: Creature) => `You face a ${c.name}, hunter. ${c.lore_summary}`,
  (c: Creature) => `The ${c.name}. ${c.lore_summary}`,
];

const IDENTIFY_TEMPLATES = [
  (matches: Creature[]) => {
    const names = matches.slice(0, 3).map(c => c.name).join(', ');
    return `Your clues point to ${names.length > 0 ? names : '... nothing definitive. Which is embarrassing. Try more details.'}`;
  },
  (matches: Creature[]) => {
    if (matches.length === 0) return 'Nothing matches those clues. And I do mean nothing. Try again.';
    if (matches.length === 1) return `That narrows to one creature: ${matches[0].name}. ${matches[0].lore_summary.split('.')[0]}.`;
    return `Several candidates, hunter. The likeliest is ${matches[0].name}. ${matches[0].lore_summary}`;
  },
];
...
const MULTI_MATCH_PREFIX = [
  (n: number) => n > 1 ? `I see ${n} possibilities. ` : '',
  (n: number) => n > 1 ? `Several candidates fit those clues. ` : '',
];
...
export function buildResponse(
  ...
  if (intent === 'identify' && allMatches.length > 1) {
    ...
    const template = pickRandom(DESCRIBE_TEMPLATES);
    answer = template(topMatch);
  }
